...
# Importing the london model
print("Importing the network ... \n")

network_path = "data/data/networks/london.gml"
node_dtypes = {'id': int}
if path.isfile(network_path):
    ...
    import data.data_queries.london_query
...
network_display_path = "test/london_test/london_network.png"
if not(path.isfile(network_display_path)):
   ox.plot.plot_graph(G, node_size = 2, node_color = '#3F4A99', edge_color = "#B0B0B0", bgcolor = '#FFFFFF', show = False, save = True, filepath = network_display_path, close = True)
else: 
...
tree_path = "data/data/percolation_trees/london_tree.npy"
...
#tree_display_path = "test/london_test/london_percolation_tree.png"
#if not(path.isfile(tree_display_path)): 
#    percolation_tree.plot(truncate_mode="level", p=10)
#    plt.title("Percolation tree of the london model")
...
cluster_path = "data/data/clusters/london/cut/" + str(threshold) + "/"
...
cut_display_path = "test/london_test/london_cut_clustering"+ str(threshold)+".png"
if path.isfile(cut_display_path) : 
    ...
    ox.plot.plot_graph(G, node_size = 2, node_color = node_colors, edge_color = "#B0B0B0", bgcolor = '#FFFFFF', show = False, save = True, filepath = cut_display_path, close = True)
...
if path.exists(condensed_tree_path):
    condensed_tree = CondensedTree(path = condensed_tree_path + "/london_condensed_tree" + str(min_size) + ".npy")
else : 
    ...
    condensed_tree.save(condensed_tree_path + "/london_condensed_tree" + str(min_size) + ".npy")


# Display the condensed tree
print("Displaying the condensed tree ... \n")
condensed_displayed_path = "test/london_test/london_condensed_tree"+str(min_size)+".png"
...
cluster_path = "data/data/clusters/london/stability/" + str(min_size) + "/"
if path.exists(cluster_path):
...
cluster_condensed_display_path = "test/london_test/london_clusters_condensed_tree"+str(min_size)+".png"
...
stability_display_path = "test/london_test/london_stability_clustering"+ str(min_size)+".png"
if path.isfile(stability_display_path) : 
    print(f"Stability clusters at min_size {min_size} had already been displayed.")
else : 
    clustering.add_clusters_to_graph(G)
    node_colors = clustering.get_node_colors()
    ox.plot.plot_graph(G, node_size = 2, node_color = node_colors, edge_color = "#B0B0B0", bgcolor = '#FFFFFF', show = False, save = True, filepath = stability_display_path, close = True)
